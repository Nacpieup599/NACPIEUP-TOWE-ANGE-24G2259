...
- FakeStore API : Utilisation de l'API REST FakeStore pour l'importation et la synchronisation des données d'articles.      Cela permet de simuler un environnement de marché réel et de tester la robustesse de l'application face à des données externes dynamiques.
- Requests (Python) : Bibliothèque utilisée pour effectuer les appels HTTP vers l'API, gérer les réponses JSON et intégrer les produits dans l'interface Streamlit.

 3)Lien de l'application en ligne
  ...
  Code Source (GitHub)
   Le code complet du projet, l'historique des versions et la documentation technique sont disponibles sur le dépôt officiel :
   [Lien vers le dépôt GitHub](https://github.com/Nacpieup599/NACPIEUP-TOWE-ANGE-24G2259)
